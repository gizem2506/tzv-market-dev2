...
const VALIDATION_SCHEMA = yup.object().shape({
  ...
  product_kdv: yup.number().default(0).required("KDV gereklidir!"),
});
    ...
    startTransition(async () => {
      try {
        ...
        if (data?.error) {
          ...
          const result = await addProductImage(data.product.id, formData);
        ...
        setError("Yetkiniz yok veya işlem sırasında bir hata oluştu.");
      }
    });
  };
        ...
        {({
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
              <H6 mb={2}>Ürün Özellikleri</H6>
                <CKEditors
                  value={values.product_description}
                  onChange={(data) => setFieldValue("product_description", data)}
                />
